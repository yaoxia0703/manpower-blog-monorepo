 ...
 * 2. 関数インデックス → 仮想カラム + 通常ユニークインデックス
 * 3. MySQL固有のヒントコメントを除去
 */
...
/* ============================================================
   t_sys_permission
   ※ 仮想カラムで code+method+is_deleted の複合ユニークを実現
   ============================================================ */
...
CREATE TABLE `t_sys_permission` (
  ...
  -- 仮想カラム：未削除の場合は code+method を結合した文字列、削除済みはNULL
  `_uk_code_method_active` varchar(115) AS (
    CASE WHEN `is_deleted` = 0 THEN CONCAT(`code`, '||', IFNULL(`method`, '')) ELSE NULL END
  ) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_code_method_active` (`_uk_code_method_active`),
  KEY `idx_parent_id`  (`parent_id`),
  KEY `idx_type`       (`type`),
  KEY `idx_path_method`(`path`,`method`)
) ENGINE=InnoDB AUTO_INCREMENT=1033
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='権限マスタ（MENU/BUTTON/API）';

INSERT INTO `t_sys_permission`
  (`id`,`parent_id`,`name`,`code`,`type`,`path`,`method`,`sort`,`status`,`created_at`,`updated_at`,`is_deleted`)
VALUES
...
/* ============================================================
   t_sys_role
   ※ is_deleted が bigint（削除時=レコードのid）のため仮想カラムで対応
   ============================================================ */
...
CREATE TABLE `t_sys_role` (
  ...
  -- 仮想カラム：未削除の場合はcode、削除済みはNULL
  `_uk_code_active` varchar(50) AS (
    CASE WHEN `is_deleted` = 0 THEN `code` ELSE NULL END
  ) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_role_code` (`_uk_code_active`)
...
INSERT INTO `t_sys_role`
  (`id`,`code`,`name`,`sort`,`status`,`created_at`,`updated_at`,`is_deleted`)
VALUES
...
CREATE TABLE `t_sys_role_menu` (
  ...
  `updated_at` datetime NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新日時',
  `is_deleted` tinyint  NOT NULL DEFAULT '0'   COMMENT '論理削除フラグ（0=未削除、1=削除済み）',
  -- 仮想カラム：未削除の場合はmenu_id、削除済みはNULL
  `_uk_menu_active` bigint AS (
    CASE WHEN `is_deleted` = 0 THEN `menu_id` ELSE NULL END
  ) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_role_menu_active`          (`role_id`, `_uk_menu_active`),
  KEY `idx_sys_role_menu_role_id`           (`role_id`),
  KEY `idx_sys_role_menu_menu_id`           (`menu_id`)
) ENGINE=InnoDB AUTO_INCREMENT=19
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='ロールメニュー関連テーブル';

INSERT INTO `t_sys_role_menu`
  (`id`,`role_id`,`menu_id`,`created_at`,`updated_at`,`is_deleted`)
VALUES
...
CREATE TABLE `t_sys_role_permission` (
  ...
  `updated_at`    datetime NOT NULL               COMMENT '更新日時',
  `is_deleted`    tinyint  NOT NULL DEFAULT '0'   COMMENT '論理削除フラグ（0=未削除、1=削除済み）',
  -- 仮想カラム：未削除の場合はpermission_id、削除済みはNULL
  `_uk_perm_active` bigint AS (
    CASE WHEN `is_deleted` = 0 THEN `permission_id` ELSE NULL END
  ) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_role_perm_active` (`role_id`, `_uk_perm_active`),
...
INSERT INTO `t_sys_user`
  (`id`,`nick_name`,`status`,`created_at`,`updated_at`,`is_deleted`)
VALUES
...
CREATE TABLE `t_sys_user_account` (
  ...
  `updated_at`    datetime     NOT NULL               COMMENT '更新日時',
  `is_deleted`    tinyint      NOT NULL DEFAULT '0'   COMMENT '論理削除フラグ（0=未削除、1=削除済み）',
  -- 仮想カラム：未削除の場合はaccount_type+account_value、削除済みはNULL
  `_uk_account_active` varchar(125) AS (
    CASE WHEN `is_deleted` = 0
         THEN CONCAT(`account_type`, '||', `account_value`)
         ELSE NULL END
  ) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_account_active` (`_uk_account_active`),
  KEY `idx_user_id` (`user_id`)
) ENGINE=InnoDB AUTO_INCREMENT=3
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci
  COMMENT='ユーザーログインアカウント';

INSERT INTO `t_sys_user_account`
  (`id`,`user_id`,`account_type`,`account_value`,`password`,`verified`,`status`,`created_at`,`updated_at`,`is_deleted`)
VALUES
...
CREATE TABLE `t_sys_user_role` (
  ...
  `updated_at` datetime NOT NULL               COMMENT '更新日時',
  `is_deleted` tinyint  NOT NULL DEFAULT '0'   COMMENT '論理削除フラグ（0=未削除、1=削除済み）',
  -- 仮想カラム：未削除の場合はrole_id、削除済みはNULL
  `_uk_role_active` bigint AS (
    CASE WHEN `is_deleted` = 0 THEN `role_id` ELSE NULL END
  ) VIRTUAL,
  PRIMARY KEY (`id`),
  UNIQUE KEY `uk_user_role_active` (`user_id`, `_uk_role_active`),
  KEY `idx_user_id` (`user_id`),
  KEY `idx_role_id` (`role_id`)
) ENGINE=InnoDB AUTO_INCREMENT=3
  DEFAULT CHARSET=utf8mb4
...
INSERT INTO `t_sys_user_role`
  (`id`,`user_id`,`role_id`,`created_at`,`updated_at`,`is_deleted`)
VALUES
