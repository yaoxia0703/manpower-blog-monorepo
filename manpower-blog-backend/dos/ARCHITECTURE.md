...
> `blog-infra` は開発時のみ使用するツール専用モジュールである。
> 依存はすべて `test` scope であり、実行時のアプリケーションには含まれない。
> 業務コードを持たないため、2.1 のパッケージ命名規約および
> `LayerDependencyTest` の対象外とする。
> 名称が `infrastructure` 層を連想させるが、レイヤとは無関係である。
...
SQL の発行方式は使い分けるが、**ページ値の補正（既定値・上限）は `PageQuery.clamped` の一箇所だけで行う**。

`PageRequest` は HTTP パラメータをそのまま受け取る入れ物であり、補正しない。

かつては `PageRequest` の生成時にも上限を適用していた。結果として上限が「コード上の定数」と「`app.page.max-page-size`」の二箇所に存在し、定数が先に働くため設定値を変更しても反映されなかった。設定が死んでいることは動作からは見えず、値を変えても何も起きないという形でしか現れない。

同様に `ServiceHelper` が content モジュール専用の定数を別に持っていた。system と content で上限が別々に定義され、片方を変えても他方が追随しない状態だった。いずれも撤去済みである。

補正を例外送出に置き換える案は採れない。`PageRequest` は Controller 到達前のバインディング時に生成されるため、ここで例外を投げても `GlobalExceptionHandler` が捕捉できず HTTP 500 になる。外部入力は拒否ではなく補正で受ける。
...
#### メソッド引数の検証は内蔵機構へ一本化する

Spring Framework 6.1 以降、Controller のメソッド引数へ直接付与した制約は、クラスへの `@Validated` を必要とせず内蔵の検証機構が処理する。両者を混在させると経路が二つに分かれる。

| 記述 | 検証機構 | 送出される例外 |
|---|---|---|
| クラスに `@Validated` | `MethodValidationPostProcessor`（AOP プロキシ） | `ConstraintViolationException` |
| 付与しない | Spring 内蔵のメソッド検証 | `HandlerMethodValidationException` |

例外型が異なるため、片方のハンドラしか無ければ応答も食い違う。実際、`@Validated` を持つ Controller だけが検証メッセージを返し、持たない Controller は同じ入力誤りをサーバエラーとして返していた。

`@Validated` を全廃し内蔵機構へ揃える。AOP プロキシの生成も不要になる。`ConstraintViolationException` のハンドラは、将来 Service 層へ `@Validated` を付ける場合に備えて残す。

#### 経路変数には `@Positive` を用いる

`@NotNull` は経路変数に対して実質的に働かない。経路が一致した時点で値は存在しており、欠落時はそもそも 404 となって Controller へ到達しない。一方 `/0` や `/-1` は経路に一致するため、`@NotNull` を素通りして処理へ到達する。

不正な ID は後段で「対象が見つからない」として弾かれるが、そのために無駄な問い合わせが発生し、エラーの意味も入力誤りから存在しないリソースへすり替わる。

数値の経路変数には `@Positive` を付与する。`LayerDependencyTest` がこれを強制する。

対象を数値型に限定するのは、`@Positive` が文字列を扱えないためである。将来 `/{code}` のような経路変数を追加した際に、実行時例外となる注釈の付与を強制してしまう。

### 11.12 機密情報と環境依存値の外部化

秘密鍵・許可オリジンのような値はコードにも共通設定にも直接書かない。

| 値 | 既定値 | 未設定時 |
|---|---|---|
| `JWT_SECRET` | 持たせない | 起動失敗 |
| `JWT_ISSUER` | 共通設定にのみ持つ（本番は上書き必須） | 起動失敗 |
| `CORS_ALLOWED_ORIGINS` | 開発向けに `localhost:5173` | 本番は上書き必須 |

秘密鍵に既定値を与えない理由は、弱い鍵のまま本番へ到達しうるためである。開発時の利便より優先する。開発用の値は git 管理外の `application-dev.yml` へ置き、`application-dev.example.yml` に必要な項目を記載する。

issuer に既定値を与えないのは、設定漏れに気付けないまま環境間でトークンが相互に通用してしまうためである。

いずれも未設定なら起動時点で失敗させる。症状が実行時まで表面化する設定は、発見が遅れるほど原因の特定が難しくなる。

> これらの設定は Spring コンテキストを起動しなければ検証できない。
> 現状 `@SpringBootTest` を持たないため CI では検出できず、
> 変更時はローカルでの起動確認が必須となる。

## 12. 今後の拡張

- Spring コンテキストの起動を検証するテストを追加する。現状 `@SpringBootTest` が
  1件も無く、設定の誤りや Bean 配線の破綻を CI で検出できない。
  「テストは緑だがアプリケーションが起動しない」状態が成立しうる。
  DataSource が必要になるため、テスト用プロファイルの分離とあわせて検討する。
- `/api/member/**` の認証除外設定を追加する。会員の登録・ログインのみ `permitAll` とし、
  その他は動的認可へ委ねる。現状 Controller が骨組みのみのため、
  実装が入る段階で設定と同時に行う（先に設定だけ入れると検証できない規則が残る）。
